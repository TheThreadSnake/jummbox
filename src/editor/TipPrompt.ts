		...
		switch (type) {
			...
			case "instrumentIndex": {
				message = div(
					...
					p("In the \"Channel Settings\" option from MicroBox's \"File\" menu, there are a couple ways to enable multiple instruments per channel, in which case you can click the + button to add more instruments to a channel."),
			...
			case "instrumentType": {
				message = div(
					h2("Instrument Type"),
					p("MicroBox comes with many instrument presets, try them out! You can also create your own custom instruments!"),
			...
			case "chipWave": {
				message = div(
					h2("Chip Wave"),
					p("MicroBox comes with some sound waves based on classic electronic sound chips, as well as several unique waves. This is the basic source of the sound of the instrument, which is modified by the other instrument settings."),
				);
			} break;
			case "chipNoise": {
				message = div(
			...
			case "effects": {
				message = div(
					h2("Effects"),
					p("MicroBox has many different kinds of special effects you can add to instruments. You can turn on multiple effects at once, and they can be configured individually. Try them all out!"),
				);
			} break;
